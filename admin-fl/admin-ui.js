...
function abrirCfg() {
  ...
  document.getElementById('cfg-path').value      = c.path   || 'front-index/produtos.json';
...
function salvarCfg() {
  var c = {
    ...
    path:      document.getElementById('cfg-path').value.trim()   || 'front-index/produtos.json',
...
async function publicar() {
  ...
  try {
    setPM('Passo 1/2','Publicando produtos.json...');
    ...
    var b64js=btoa(unescape(encodeURIComponent(gerarJSON())));
    ...
    document.getElementById('codeArea').textContent=gerarJSON(); abrirModal('modalExport'); return;
  }
  if(!autoIndex){
...
function bind() {
  ...
  document.getElementById('btn-exportar').addEventListener('click', function(){
    document.getElementById('codeArea').textContent=gerarJSON(); abrirModal('modalExport');
  });
